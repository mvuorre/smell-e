...
p1 <- dat |> 
  ...
  #   shape = 1,
  #   alpha = 0.25,
  #   position = position_quasirandom(width = .1, dodge.width = .4)
  # ) +
  stat_summary(
    fun.data = mean_se,
    size = 0.55,
...
bind_rows(
  ...
  tibble() |> 
  ggplot(aes(Parameter, Mean, fill = pd > 0.975)) +
...
#| eval: false
#| label: fig-dag
#| fig-cap: "Schematic of the proposed mediation model. E: Exposure, P: Presence, dFCR: Difference in food cue response."
...
coordinates(g) <- list(
  x = c(E = 0, P = 1, dFCR = 2),
  y = c(E = 0, P = -0.1, dFCR = 0)
)
plot(g)
```

We then turned to examine the potential psychological processes that might mediate the effects of VR exposure on food cue responses. To do so, we first calculated ΔFCR = FCR_Food - FCR_Nonfood for each participant. Then, we specified a multivariate regression model predicting presence from VR exposure mode (UVR / MVR), and one predicting ΔFCR from presence. We specified both paths with by-person random intercepts. We then quantified whether presence mediates FCR-enhancing effects of the multisensory VR condition by multiplying the VR exposure mode (unisensory VR vs multisensory VR) -> presence path coefficient with the presence -> FCR path coefficient.

We show the results in @tbl-h2 and @fig-h2: While the multisensory VR exposure led to much greater feelings of presence, presence did not credibly predict either craving or salivary volume ΔFCR.
...
dat2 <- dat |> 
  ...
  # Aggregate state_i to exposures so can use in mediation
  mutate(state_i = mean(state_i), .by = c(participant_id, exposure)) |> 
  pivot_wider(
    names_from = stimulus, 
    values_from = c(craving, volume)
  ) |> 
  mutate(
    d_craving = craving_F - craving_NF,
    d_volume = volume_F - volume_NF, 
    .keep = "unused"
  )
...
hypotheses_craving <- c(
  ...
  `P->ΔFCR` = "dcraving_presence = 0",
  `E->P X P->ΔFCR` = "presence_exposureMVR * dcraving_presence = 0",
  Prop_mediated = "(presence_exposureMVR * dcraving_presence) / 
    (presence_exposureMVR * dcraving_presence + dcraving_exposureMVR) = 0"
)
...
hypotheses_volume <- hypotheses_craving |> 
  str_replace_all("dcraving", "dvolume") |> 
...
p1 <- plot(
  ...
  labs(x = "Presence", y = "ΔFCR [Craving]") |
  plot(
    cc3, 
    plot = FALSE,
    line_args = list(color = "black", linewidth = 0.5)
  )[[1]] +
  labs(x = "Presence", y = "ΔFCR [Volume]")
...
We then turned to our more exploratory analyses. Our first aim was to examine whether state gustatory mental imagery might mediate any effects of VR exposure mode on ΔFCRs. To do so, we first took the mean imagery for each participant in each VR exposure mode. Then, we conducted mediation analyses as above (Hypothesis 2) but used imagery as the mediator rather than presence. We show the results in @tbl-e1 and @fig-e1: While the multisensory VR exposure led to elevated imagery, imagery did not credibly predict either craving or salivary volume ΔFCR.
...
hypotheses_craving <- c(
  `E->P` = "statei_exposureMVR = 0",
  `P->ΔFCR` = "dcraving_state_i = 0",
  `E->P X P->ΔFCR` = "statei_exposureMVR * dcraving_state_i = 0",
  Prop_mediated = "(statei_exposureMVR * dcraving_state_i) / 
    (statei_exposureMVR * dcraving_state_i + dcraving_exposureMVR) = 0"
)
...
p1 <- plot(
  ...
  labs(x = "State imagery", y = "ΔFCR [Craving]") |
  plot(
    cc3, 
    plot = FALSE,
    line_args = list(color = "black", linewidth = 0.5)
  )[[1]] +
  labs(x = "State imagery", y = "ΔFCR [Volume]")
...
fit_volume_e2o <- brm(
  ...
  dat,
  iter = ITER,
  file = "models/brm-volume-e2o"
)

tibble(
  Outcome = rep(c("Craving", "Volume"), each = 2),
  Imagery = rep(c("Gustatory", "Olfactory"), 2),
  model = list(fit_craving_e2g, fit_craving_e2o, fit_volume_e2g, fit_volume_e2o)
) |> 
